...
const Navbar = () => {
  ...
  return (
    <>
      <div className=" fixed z-[999] w-full  flex justify-between  sm:px-5 sm:py-5 md:px-20 md:py-8  items-center">
        ...
        <div className=" visible-links hidden  md:flex gap-10 ">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a href="/" key={index} className={`${index === 4 && "ml-32"}`}>
              {item}
            </a>
          )
        )}
        ...
        <div onClick={handleMenu} className="menu-icon md:hidden text-3xl">
          {isOpen ?<FaXmark/> : <IoMdMenu />  }
        </div>

      </div>

     {isOpen && (<div className="hidden-links sm: w-[70%]  h-[100vh]   bg-black z-[999] left-0 top-0  absolute opacity-[0.8]">
        <nav class="flex flex-col pt-5 gap-3   ">
          <a class=" flex justify-center text-black font-bold text-xl -tracking-tighter hover:bg-gray-600 bg-zinc-50 py-3" href="/">
            Services
          </a>
          <a class=" flex justify-center text-black font-bold text-xl -tracking-tighter hover:bg-gray-600 bg-zinc-50  py-3" href="/">
            Our Work
